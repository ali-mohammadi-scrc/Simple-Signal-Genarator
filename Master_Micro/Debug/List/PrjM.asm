...
; 0000 0021 /* Wait for empty transmit buffer */
; 0000 0022     while ( !( UCSRA & (1<<UDRE)) );
	ST   -Y,R26
;	data -> Y+0
_0x3:
	SBIS 0xB,5
	RJMP _0x3
; 0000 0023 /* Put data into buffer, sends the data */
; 0000 0024     UDR = data;
	LD   R30,Y
	OUT  0xC,R30
; 0000 0025 }
...
;void UseKey (int In, int Row){
; 0000 0027 void UseKey (int In, int Row){
_UseKey:
; .FSTART _UseKey
; 0000 0028     char Data;
; 0000 0029     if (In == 4) In = 3;
	ST   -Y,R27
	ST   -Y,R26
	ST   -Y,R17
;	In -> Y+3
...
;
;void KeyPad (){
; 0000 002E void KeyPad (){
_KeyPad:
; .FSTART _KeyPad
; 0000 002F     char In;
; 0000 0030     char Check;
; 0000 0031     In = ~PINC & 0b00000111;
	ST   -Y,R17
	ST   -Y,R16
;	In -> R17
;	Check -> R16
	...
	ANDI R30,LOW(0x7)
	MOV  R17,R30
; 0000 0032     PORTD ^= 0b00011000; //ROW C&D
	IN   R30,0x12
	LDI  R26,LOW(24)
	CALL SUBOPT_0x0
; 0000 0033     delay_us (10);
; 0000 0034     Check = ~PINC & 0b00000111;
; 0000 0035     if (Check == In){
	CP   R17,R16
	BRNE _0x7
; 0000 0036         PORTD ^= 0b01000000; //ROW D
	IN   R30,0x12
	LDI  R26,LOW(64)
	EOR  R30,R26
	OUT  0x12,R30
; 0000 0037         Check = ~PINC & 0b00000111;
	IN   R30,0x13
	COM  R30
	ANDI R30,LOW(0x7)
	MOV  R16,R30
; 0000 0038         if (Check == In) UseKey (In, 3);
	CP   R17,R16
	BRNE _0x8
	CALL SUBOPT_0x1
	LDI  R26,LOW(3)
	RJMP _0x11
; 0000 0039         else UseKey (In, 2);
_0x8:
	CALL SUBOPT_0x1
	LDI  R26,LOW(2)
_0x11:
...
; 0000 003B     else {
	RJMP _0xA
_0x7:
; 0000 003C         PORTD ^= 0b11010000; //ROW B
	IN   R30,0x12
	LDI  R26,LOW(208)
	CALL SUBOPT_0x0
; 0000 003D         delay_us (10);
; 0000 003E         Check = ~PINC & 0b00000111;
; 0000 003F         if (Check) UseKey (In, 1);
	CPI  R16,0
	BREQ _0xB
	CALL SUBOPT_0x1
	LDI  R26,LOW(1)
	RJMP _0x12
; 0000 0040         else UseKey (In, 0);
_0xB:
	CALL SUBOPT_0x1
	LDI  R26,LOW(0)
_0x12:
...
; 0000 0064 PORTC=(0<<PORTC7) | (0<<PORTC6) | (0<<PORTC5) | (0<<PORTC4) | (0<<PORTC3) | (1<<PORTC2) | (1<<PORTC1) | (1<<PORTC0);
	LDI  R30,LOW(7)
	OUT  0x15,R30
; 0000 0065 
; 0000 0066 // Port D initialization
; 0000 0067 // Function: Bit7=In Bit6=In Bit5=In Bit4=In Bit3=In Bit2=In Bit1=In Bit0=In
; 0000 0068 DDRD=(1<<DDD7) | (1<<DDD6) | (0<<DDD5) | (1<<DDD4) | (1<<DDD3) | (0<<DDD2) | (0<<DDD1) | (0<<DDD0);
	LDI  R30,LOW(216)
	OUT  0x11,R30
...
; 0000 0071 TCCR0=(0<<WGM00) | (0<<COM01) | (0<<COM00) | (0<<WGM01) | (0<<CS02) | (0<<CS01) | (0<<CS00);
	OUT  0x33,R30
...
; 0000 009A // INT0: On
; 0000 009B // INT0 Mode: Low level
; 0000 009C // INT1: Off
; 0000 009D // INT2: Off
; 0000 009E GICR|=(0<<INT1) | (1<<INT0) | (0<<INT2);
	IN   R30,0x3B
	ORI  R30,0x40
	OUT  0x3B,R30
; 0000 009F MCUCR=(0<<ISC11) | (0<<ISC10) | (0<<ISC01) | (0<<ISC00);
	LDI  R30,LOW(0)
	OUT  0x35,R30
; 0000 00A0 MCUCSR=(0<<ISC2);
	OUT  0x34,R30
; 0000 00A1 GIFR=(0<<INTF1) | (1<<INTF0) | (0<<INTF2);
	LDI  R30,LOW(64)
	OUT  0x3A,R30
; 0000 00A2 
; 0000 00A3 // USART initialization
; 0000 00A4 // Communication Parameters: 8 Data, 1 Stop, No Parity
; 0000 00A5 // USART Receiver: Off
; 0000 00A6 // USART Transmitter: On
; 0000 00A7 // USART Mode: Asynchronous
; 0000 00A8 // USART Baud Rate: 9600
; 0000 00A9 UCSRA=(0<<RXC) | (0<<TXC) | (0<<UDRE) | (0<<FE) | (0<<DOR) | (0<<UPE) | (0<<U2X) | (0<<MPCM);
	LDI  R30,LOW(0)
	OUT  0xB,R30
; 0000 00AA UCSRB=(0<<RXCIE) | (0<<TXCIE) | (0<<UDRIE) | (0<<RXEN) | (1<<TXEN) | (0<<UCSZ2) | (0<<RXB8) | (0<<TXB8);
	LDI  R30,LOW(8)
	OUT  0xA,R30
; 0000 00AB UCSRC=(1<<URSEL) | (0<<UMSEL) | (0<<UPM1) | (0<<UPM0) | (0<<USBS) | (1<<UCSZ1) | (1<<UCSZ0) | (0<<UCPOL);
	LDI  R30,LOW(134)
	OUT  0x20,R30
; 0000 00AC UBRRH=0x00;
	LDI  R30,LOW(0)
	OUT  0x20,R30
; 0000 00AD UBRRL=0x33;
	LDI  R30,LOW(51)
	OUT  0x9,R30
; 0000 00AE 
